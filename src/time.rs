...
const fn normalize_alternative_modifier_token(modifier: u8, token: u8) -> Option<u8> {
  match modifier {
    b'E' => match token {
      b'c' | b'C' | b'x' | b'X' | b'y' | b'Y' | b'z' | b'u' | b'r' | b'R' | b'p' | b'T' => {
        Some(token)
      }
      _ => None,
    },
...
///   `%Ec/%EC/%Ex/%EX/%Ey/%EY/%Ez/%Eu/%Ep/%Er/%ER/%ET` and
///   `%OC/%Od/%Oe/%OH/%OI/%Oj/%Ok/%Ol/%Om/%OM/%OS/%Ou/%OU/%OV/%Ow/%OW/%Oy/%OY/%OG/%Og/%Op/%OP/%OR/%OT/%Or/%Oz`.

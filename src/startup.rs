...
mod tests {
  ...
  #[test]
  fn startup_main_preserves_repeated_valid_entries_with_mixed_skips() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    assert_eq!(status, 77);
    assert_eq!(snapshot_calls(), vec![1, 1, 2, 3, 5, 4, 4]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_storage.as_mut_ptr() as usize,
    );
  ...
  #[test]
  fn startup_main_preserves_distinct_repeated_entries_between_init_and_fini_ranges() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    // SAFETY: ranges are contiguous pointer-sized entries. Null and
    // misaligned non-null slots should be skipped while each range preserves
    // its own repeated valid entry sequence.
    let status =
      unsafe { run_startup_main(test_main as StartMainFn, args, init_range, fini_range) };

    assert_eq!(status, 77);
    assert_eq!(snapshot_calls(), vec![1, 1, 2, 3, 4, 5, 5]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_storage.as_mut_ptr() as usize,
    );
  ...
  #[test]
  fn startup_main_preserves_alternating_repeated_entries_with_mixed_skips() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    // SAFETY: ranges are contiguous pointer-sized entries. Null and
    // misaligned non-null slots should be skipped while preserving alternating
    // repeated valid entries.
    let status =
      unsafe { run_startup_main(test_main as StartMainFn, args, init_range, fini_range) };

    assert_eq!(status, 77);
    assert_eq!(snapshot_calls(), vec![1, 2, 1, 2, 3, 5, 4, 5, 4]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_storage.as_mut_ptr() as usize,
    );
  ...
  #[test]
  fn startup_main_preserves_alternating_repeated_entries_with_distinct_fini_pattern() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    // SAFETY: ranges are contiguous pointer-sized entries. Null and
    // misaligned non-null slots should be skipped while preserving alternating
    // repeated entries for each distinct init/fini pattern.
    let status =
      unsafe { run_startup_main(test_main as StartMainFn, args, init_range, fini_range) };

    assert_eq!(status, 77);
    assert_eq!(snapshot_calls(), vec![1, 2, 1, 2, 3, 4, 5, 4, 5]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_storage.as_mut_ptr() as usize,
    );
  ...
  #[test]
  fn libc_start_main_path_preserves_distinct_repeated_entries_between_init_and_fini_ranges() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    let unwind = panic::catch_unwind(|| {
      // SAFETY: ranges are contiguous pointer-sized entries. Null and
      // misaligned non-null slots should be skipped while preserving each
      // range's distinct repeated valid entries.
      unsafe {
        run_libc_start_main_with(
          Some(test_main as StartMainFn),
          args,
    ...
    assert_eq!(OBSERVED_ARGV.load(Ordering::Relaxed), argv_ptr as usize);
    assert_eq!(OBSERVED_ENVP.load(Ordering::Relaxed), envp_ptr as usize);
    assert_eq!(snapshot_calls(), vec![1, 1, 2, 3, 4, 5, 5, 6]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_ptr as usize,
    );
  ...
  #[test]
  fn libc_start_main_path_preserves_alternating_repeated_entries_with_mixed_skips() {
    let _test_guards = lock_test();
    let _environ_restore = EnvironRestore::capture();
    ...
    let unwind = panic::catch_unwind(|| {
      ...
      // alternating repeated valid entries.
      unsafe {
        run_libc_start_main_with(
          Some(test_main as StartMainFn),
          args,
    ...
    assert_eq!(OBSERVED_ARGV.load(Ordering::Relaxed), argv_ptr as usize);
    assert_eq!(OBSERVED_ENVP.load(Ordering::Relaxed), envp_ptr as usize);
    assert_eq!(snapshot_calls(), vec![1, 2, 1, 2, 3, 5, 4, 5, 4, 6]);
    assert_eq!(
      OBSERVED_ENVIRON_DURING_INIT.load(Ordering::Relaxed),
      envp_ptr as usize,
    );

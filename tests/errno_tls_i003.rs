...
fn three_level_second_leaf_starts_zero_after_first_leaf_exit() {
  ...
  let trunk_summary = thread::spawn(|| {
    ...
    let branch_summary = thread::spawn(|| {
      let branch_slot = __errno_location() as usize;
    ...
    (trunk_slot, trunk_start, trunk_finish, branch_summary)
  })
  .join()
  .expect("trunk thread panicked");
  let (trunk_slot, trunk_start, trunk_finish, branch_summary) = trunk_summary;
  let (branch_slot, branch_start, branch_finish, first_leaf_summary, second_leaf_summary) =
    branch_summary;
  ...
  assert_eq!(branch_start, 0, "branch thread must start with zero errno");
  assert_eq!(
    first_leaf_start, 0,
  ...
  assert_eq!(
    ...
    "second leaf thread must still observe zero before its first write",
  );
  assert_eq!(
    first_leaf_finish, 3456,
    "first leaf thread must preserve its own errno write",
  );
  assert_eq!(
    second_leaf_finish, -4567,
  ...
  assert_ne!(
    first_leaf_slot, branch_slot,
    "first leaf thread must not share errno storage with branch thread",
  );
  assert_ne!(
    second_leaf_slot, branch_slot,
    "second leaf thread must not share errno storage with branch thread",
  );
  assert_eq!(
    __errno_location() as usize,
    main_slot,
    "main-thread errno pointer must remain stable across sequential leaf spawns",
  );
  assert_eq!(
    read_errno(),
    9292,
...
fn three_level_many_sequential_leaves_start_zero_and_keep_isolation() {
  ...
  let trunk_summary = thread::spawn(|| {
    ...
    let branch_summary = thread::spawn(|| {
      ...
      (branch_slot, branch_start, branch_finish, leaf_reports)
    })
    .join()
    .expect("branch thread panicked");
    let trunk_finish = read_errno();
  ...
  .expect("trunk thread panicked");
  let (trunk_slot, trunk_start, trunk_finish, branch_summary) = trunk_summary;
  let (branch_slot, branch_start, branch_finish, leaf_reports) = branch_summary;
  ...
  assert_eq!(
    leaf_reports.len(),
    LEAF_COUNT,
    "expected one report per sequential leaf thread",
  );
  assert_eq!(
    trunk_finish, 1414,
    "branch/leaf writes must not clobber trunk errno",
    ...
    (LEAF_BASE..).take(LEAF_COUNT).zip(leaf_reports.iter())
  {
    assert_eq!(
      *leaf_start, 0,
      "each sequential leaf thread must start with zero errno",
    );
    assert_eq!(
      *leaf_before, 0,
      "each sequential leaf thread must observe zero before first write",
    );
    assert_eq!(
      *leaf_finish, expected_errno,
      "leaf thread must preserve its own errno write",
    );
    assert_ne!(
      *leaf_slot, main_slot,
    ...
    assert_ne!(
      *leaf_slot, trunk_slot,
      "leaf thread must not alias trunk-thread errno storage",
    );
    assert_ne!(
      *leaf_slot, branch_slot,
  ...
  assert_eq!(
    __errno_location() as usize,
    main_slot,
    "main-thread errno pointer must remain stable across sequential leaf generations",
  );
  assert_eq!(
    read_errno(),
    -9191,
...
#[test]
fn three_level_branch_regeneration_keeps_boundary_writes_isolated() {
  const TRUNK_FIRST: c_int = c_int::MAX - 17;
  const TRUNK_SECOND: c_int = c_int::MIN + 99;
  const FIRST_BRANCH_WRITE: c_int = c_int::MIN + 19;
  ...
  let trunk_summary = thread::spawn(|| {
    ...
    write_errno(TRUNK_FIRST);

    let first_branch_summary = thread::spawn(|| {
      let first_branch_slot = __errno_location() as usize;
      let first_branch_start = read_errno();

      write_errno(FIRST_BRANCH_WRITE);

      let first_leaf_summary = thread::spawn(|| {
        let first_leaf_slot = __errno_location() as usize;
        let first_leaf_start = read_errno();

        write_errno(FIRST_LEAF_WRITE);
    ...
    let second_branch_summary = thread::spawn(|| {
      ...
      write_errno(SECOND_BRANCH_WRITE);
  ...
  assert_eq!(
    second_branch_before, 0,
    "second branch must observe zero before first write",
  );
  assert_eq!(
    first_leaf_start, 0,
    "first leaf thread must start with zero errno",
  ...
  assert_eq!(
    second_leaf_before, 0,
    "second leaf must observe zero before first write",
  );
  assert_eq!(
    first_leaf_finish, FIRST_LEAF_WRITE,
    "first leaf thread must preserve its boundary write",
  );
  assert_eq!(
    second_leaf_finish, SECOND_LEAF_WRITE,
    "second leaf thread must preserve its boundary write",
  );
  assert_eq!(
    first_branch_finish, FIRST_BRANCH_WRITE,
    "first branch thread must preserve its boundary write",
  );
  assert_eq!(
    second_branch_finish, SECOND_BRANCH_WRITE,
    "second branch thread must preserve its boundary write",
  );
  assert_eq!(
    trunk_finish, TRUNK_SECOND,
    "branch and leaf writes must not clobber trunk boundary value",
  );
  assert_ne!(
    trunk_slot, main_slot,
    "trunk thread must not alias main-thread errno storage",
  ...
  assert_ne!(
    first_leaf_slot, first_branch_slot,
    "first leaf thread must not alias first-branch errno storage",
  );
  assert_ne!(
    second_leaf_slot, second_branch_slot,
    "second leaf thread must not alias second-branch errno storage",
  );
  assert_eq!(
    __errno_location() as usize,
    main_slot,
    "main-thread errno pointer must remain stable across boundary branch regeneration",
  );
  assert_eq!(
    read_errno(),
    -9000,
...
#[test]
fn three_level_many_branch_generations_start_zero_and_preserve_trunk_errno() {
  ...
  let trunk_summary = thread::spawn(|| {
    ...
    write_errno(TRUNK_WRITE);

    let mut round_reports = Vec::new();

    for (&branch_target, &leaf_target) in BRANCH_TARGETS.iter().zip(LEAF_TARGETS.iter()) {
      let branch_summary = thread::spawn(move || {
        let branch_slot = __errno_location() as usize;
        let branch_start = read_errno();
        let branch_before = read_errno();

        write_errno(branch_target);
      ...
      .join()
      .expect("branch thread panicked");
      let trunk_after_round = read_errno();
  ...
  assert_eq!(trunk_start, 0, "trunk thread must start with zero errno");
  assert_eq!(
    trunk_finish, TRUNK_WRITE,
    "branch/leaf writes across rounds must not clobber trunk errno",
  );
  assert_eq!(
    round_reports.len(),
    BRANCH_TARGETS.len(),
  ...
  for (expected_branch, expected_leaf, trunk_after_round, branch_summary) in &round_reports {
    ...
    assert_eq!(
      *branch_start, 0,
      "each regenerated branch must start with zero errno",
    );
    assert_eq!(
      *branch_before, 0,
      "each regenerated branch must observe zero before first write",
    );
    assert_eq!(
      *branch_finish, *expected_branch,
      "branch thread must preserve its round-specific errno write",
    );
    assert_eq!(
      *trunk_after_round, TRUNK_WRITE,
      "branch/leaf writes must not clobber trunk errno per round",
    );
    assert_eq!(
      *leaf_start, 0,
      "leaf thread must start with zero errno in each round",
    );
    assert_eq!(
      *leaf_before, 0,
      "leaf thread must observe zero before first write in each round",
    );
    assert_eq!(
      *leaf_finish, *expected_leaf,
      "leaf thread must preserve its round-specific errno write",
    );
    assert_ne!(
      *branch_slot, main_slot,
      "branch thread must not alias main-thread errno storage",
    ...
    assert_ne!(
      *branch_slot, trunk_slot,
      "branch thread must not alias trunk-thread errno storage",
    );
    assert_ne!(
      *leaf_slot, main_slot,
      "leaf thread must not alias main-thread errno storage",
  ...
  assert_eq!(
    __errno_location() as usize,
    main_slot,
    "main-thread errno pointer must remain stable across many branch generations",
  );
  assert_eq!(
    read_errno(),
    6060,

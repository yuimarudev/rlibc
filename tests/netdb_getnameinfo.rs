...
#[test]
fn getnameinfo_rejects_unknown_flags_for_service_only_short_addrlen_name_required_numerichost_request()
 {
  let address = sockaddr_in {
    sin_family: af_inet_family(),
    sin_port: 53_u16.to_be(),
  ...
  // SAFETY: service output is writable/requested. Even with
  // NI_NAMEREQD|NI_NUMERICHOST, unknown flags must be rejected before
  // short-addrlen validation.
  let status = unsafe {
    getnameinfo(
      (&raw const address).cast::<sockaddr>(),
      slen(size_of::<sockaddr_in>() - 1),
...
#[test]
fn getnameinfo_rejects_unknown_flags_before_no_output_noname_with_numerichost() {
  // SAFETY: address/output pointers are intentionally null. Unsupported flags
  // must be rejected before no-output handling even when NI_NUMERICHOST is set.
  let status = unsafe {
    getnameinfo(
      core::ptr::null(),
      slen(0),
...
#[test]
fn getnameinfo_allows_service_only_request_with_name_required_and_numerichost_flags() {
  let address = sockaddr_in {
    sin_family: af_inet_family(),
  ...
  // SAFETY: pointers refer to valid address/service buffers; host output is
  // intentionally not requested and NI_NUMERICHOST must not affect service-only
  // numeric formatting.
  let status = unsafe {
    getnameinfo(
      (&raw const address).cast::<sockaddr>(),

...
#[test]
fn clock_gettime_monotonic_success_after_near_max_positive_invalid_clock_id_failure_keeps_errno_einval()
 {
  let mut invalid_clock_ts = timespec {
    tv_sec: -1,
    tv_nsec: -1,
...
#[test]
fn gettimeofday_success_after_clock_gettime_null_timespec_with_near_max_positive_invalid_clock_id_keeps_errno_efault()
 {
  let mut tv = timeval {
    tv_sec: -1,
    tv_usec: -1,
  ...
  let fail_rc = clock_gettime(c_int::MAX - 1, ptr::null_mut());

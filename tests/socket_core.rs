...
#[test]
fn accept_nonblocking_without_pending_connection_with_null_addr_and_non_null_addrlen_returns_minus_one_and_errno_eagain()
 {
  let socket_path = unique_socket_path();
  let address = sockaddr_un_for_path(&socket_path);
  let address_len = to_socklen(core::mem::size_of::<SockaddrUn>());
  ...
  assert!(
    server_fd >= 0,
    "socket(server null addr, non-null addrlen) failed with errno={}",
    errno_value()
  );

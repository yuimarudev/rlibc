...
fn fcntl_dupfd_cloexec_invalid_fd_with_excessive_minimum_overwrites_errno_with_ebadf() {
  ...
  // SAFETY: command does not dereference pointers and intentionally passes an invalid fd.
  let result = unsafe { fcntl(-1, F_DUPFD_CLOEXEC, as_c_long(c_int::MAX)) };
...
#[test]
fn fcntl_dupfd_cloexec_invalid_fd_with_excessive_minimum_sets_ebadf() {
  write_errno(0);
...
#[test]
fn fcntl_dupfd_cloexec_with_minimum_equal_to_source_fd_keeps_errno_unchanged() {
  let (path, file) = create_read_only_temp_file(b"dup-cloexec-min-equal-errno");
  let source_fd = file.as_raw_fd();
  ...
  // SAFETY: `duplicated_fd` is freshly returned by `F_DUPFD_CLOEXEC`

...
#[test]
fn memcpy_zero_length_allows_same_empty_array_pointer_with_distinct_alignment_origin() {
  let mut buffer_words: [u16; 0] = [];
  let pointer = buffer_words.as_mut_ptr().cast::<u8>();
  // SAFETY: `n == 0`, so no memory access is performed.
  let returned = unsafe { memcpy(pointer.cast(), pointer.cast_const().cast(), sz(0)) }.cast::<u8>();

  assert_eq!(returned, pointer);
  assert_eq!(buffer_words, []);
}

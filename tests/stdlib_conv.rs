...
#[test]
fn explicit_hex_prefixed_overflow_with_null_endptr_sets_erange() {
  let big_prefixed_hex_digits = b"0xffffffffffffffffffffffffffffffff\0";

  set_errno(351);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let alpha_overflow = unsafe { strtol(big_prefixed_hex_digits.as_ptr().cast(), null_mut(), 16) };
  ...
  set_errno(352);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let beta_overflow = unsafe { strtoll(big_prefixed_hex_digits.as_ptr().cast(), null_mut(), 16) };
  ...
  set_errno(353);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let gamma_overflow = unsafe { strtoul(big_prefixed_hex_digits.as_ptr().cast(), null_mut(), 16) };
  ...
  set_errno(354);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let delta_overflow = unsafe { strtoull(big_prefixed_hex_digits.as_ptr().cast(), null_mut(), 16) };
...
#[test]
fn explicit_hex_prefixed_negative_overflow_with_null_endptr_sets_erange() {
  let big_prefixed_negative_hex_digits = b"-0xffffffffffffffffffffffffffffffff\0";

  set_errno(361);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let alpha_underflow = unsafe {
    strtol(
      big_prefixed_negative_hex_digits.as_ptr().cast(),
      null_mut(),
      16,
    )
  };
...
#[test]
fn explicit_hex_uppercase_prefixed_negative_overflow_with_null_endptr_sets_erange() {
  let big_uppercase_prefixed_negative_hex_digits = b"-0XFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF\0";

  set_errno(381);
  // SAFETY: input pointer is valid and NUL-terminated; null endptr is allowed.
  let alpha_underflow = unsafe {
    strtol(
      big_uppercase_prefixed_negative_hex_digits.as_ptr().cast(),
      null_mut(),
      16,
    )
  };

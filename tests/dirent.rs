...
#[test]
fn opendir_missing_path_with_dot_slash_suffix_returns_null_and_errno_enoent() {
  let temp_dir = TempDir::new("i036-missing-dot-slash");
  let missing = temp_dir.path().join("does-not-exist");
  let mut missing_with_dot_slash = missing.as_os_str().as_bytes().to_vec();

  missing_with_dot_slash.extend_from_slice(b"/./");

  let missing_c =
    CString::new(missing_with_dot_slash).expect("missing path with dot-slash suffix must be valid");
...
#[test]
fn opendir_regular_file_with_dot_dot_suffix_returns_null_and_errno_enotdir() {
  let temp_dir = TempDir::new("i036-notdir-dot-dot");
  let regular_file = temp_dir.path().join("regular.txt");
  let mut regular_file_with_dot_dot = regular_file.as_os_str().as_bytes().to_vec();

  fs::write(&regular_file, b"content").expect("failed to create regular file for opendir test");
  regular_file_with_dot_dot.extend_from_slice(b"/..");

  let regular_file_c =
    CString::new(regular_file_with_dot_dot).expect("regular file path with dot-dot must be valid");
...
#[test]
fn opendir_regular_file_with_dot_dot_slash_suffix_returns_null_and_errno_enotdir() {
  let temp_dir = TempDir::new("i036-notdir-dot-dot-slash");
  let regular_file = temp_dir.path().join("regular.txt");
  let mut regular_file_with_dot_dot_slash = regular_file.as_os_str().as_bytes().to_vec();

  fs::write(&regular_file, b"content").expect("failed to create regular file for opendir test");
  regular_file_with_dot_dot_slash.extend_from_slice(b"/../");

  let regular_file_c = CString::new(regular_file_with_dot_dot_slash)
    .expect("regular file path with dot-dot-slash must be valid");

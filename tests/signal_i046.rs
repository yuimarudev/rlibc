...
#[test]
fn kill_with_process_group_target_and_negative_signal_returns_einval() {
  let _lock = signal_lock();

  write_errno(0);

  let status = kill(0, NEGATIVE_SIGNAL);
...
#[test]
fn kill_with_process_group_target_and_first_out_of_range_signal_returns_einval() {
  let _lock = signal_lock();

  write_errno(0);

  let status = kill(0, FIRST_OUT_OF_RANGE_SIGNAL);
...
#[test]
fn kill_with_broadcast_target_and_negative_signal_returns_einval() {
  let _lock = signal_lock();

  write_errno(0);

  let status = kill(-1, NEGATIVE_SIGNAL);

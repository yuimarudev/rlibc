...
#[test]
fn count_conversion_later_unsupported_specifier_with_ln_and_lln_keeps_prior_successful_writes() {
  let mut buffer = [b'Q'; 16];
  ...
  set_errno(0);
  // SAFETY: `%ln`/`%lln` pointers are valid; `%q` is intentionally unsupported for error-path coverage.
  let result = unsafe {
    vsnprintf(
      buffer.as_mut_ptr().cast(),
      as_size_t(buffer.len()),
      as_format_ptr(b"ab%lnC%lln%qZ\0"),
      ap.as_mut_ptr(),
    )
  };
